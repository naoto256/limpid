...
- **Recovered** (any queue). The failure was durably captured. Either `control { error_log }` is set and the DLQ file write succeeded — the event is persisted as an Output-flavor record ready for replay (see [Error Log → Producer sites](./error-log.md#producer-sites)) — or `error_log` is unset and the tracing-side fallback ran per the operator's `error_log_fallback` [ladder](./error-log.md#tracing-fallback-ladder-error_log_fallback) (default: one-line summary, no payload; opt-in required for metadata / full JSONL). The `--check` recovery-readiness warning flags the no-`error_log` case at configuration time so an operator who wanted file-based recovery notices the missing setting before the first failure.
