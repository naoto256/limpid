...
The path must be in a directory the daemon user can write to. Daemon startup validates this by actually opening the DLQ file — `O_CREAT|O_EXCL` with `mode(0o600)` if it does not exist yet, `O_RDWR|O_APPEND|O_NOFOLLOW|O_NONBLOCK` if it does — and refuses to start if the open fails with `EACCES`, `EPERM`, `EROFS`, an SELinux / AppArmor confinement, or any other reason a bare parent-directory stat would miss. A successful preflight against an absent path leaves an empty 0o600 file at the configured path — that is the same state the runtime would have produced on the first real failure, so no observable behavior changes but the failure surfaces up front rather than at first DLQ write. (`limpid --check` does not run this preflight; configuration validation must not touch the filesystem beyond a read-only stat.)
