...
**Scope of the wedge contract**: the fail-stop wedge is defined for **unbatched sinks** (`file`, `stdout`, `unix_socket`, `syslog_tcp`, `syslog_udp`, `kafka`). Unbatched sinks resolve each event's ack synchronously inside their `consume` call, so `in-flight == 0` at wedge time and the consumer drains cleanly. **Batched sinks** (`http`, `otlp_http`, `otlp_grpc`) accept events into their own internal `(Event, QueueAckHandle)` buffer before flushing — if the wedge fires while such a buffer holds parked handles, the buffered handles do **not** currently receive an explicit resolve at shutdown (the drain loop skips `writer.shutdown()` to avoid pumping more events through a bug-path output), and the consumer waits on ack messages that will never arrive. In practice this shape is rare: batched-sink Dropped comes from a bug or panic that would already have shut the actor down, so the batched buffer is usually empty at wedge time. Extending `writer.shutdown()` with a wedge-aware resolve is queued for a follow-up release; until then, operators seeing a batched-sink consumer that fails to exit after wedge should send `SIGKILL` to force the daemon out and rely on the disk queue's cursor to replay on next start.
