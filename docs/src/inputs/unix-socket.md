...
- **Parent-directory safety** is enforced at daemon startup as two independent checks on the configured `path`'s parent directory:
  - **Owner** — the parent must be owned by root (uid 0) or by the daemon's own effective uid. An untrusted owner retains rename/unlink rights inside the directory regardless of mode bits, and can swap the socket inode between the stale-cleanup stat and the follow-up unlink, or between shutdown and next bind. `/dev` (root-owned) and a daemon-owned runtime directory pass; a user-created dir owned by an unrelated uid fails independent of mode.
  ...
  Failure surfaces at startup with a diagnostic naming whether the owner or the mode failed and the observed value.
