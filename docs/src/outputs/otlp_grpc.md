...
- On **graceful shutdown** (`SIGTERM`, `SIGHUP` reload, `systemctl stop`, or an explicit `shutdown()` API call), a bounded final drain runs — one flush attempt per parked payload bounded by `SHUTDOWN_FLUSH_ATTEMPT_TIMEOUT` (3 s) plus the per-actor in-flight cancel. Drain failures route the buffered events to `control { error_log "..." }` as Output-flavor DLQ records with the `Recovered` marker — `event.source`, `event.received_at`, `event.ingress`, and `event.egress` reflect the original per-event provenance because the batched output parks the source `Event` alongside its `QueueAckHandle` until the flush resolves. Without `error_log` the daemon emits one `tracing::error!` per drain-failure event with the full failure JSONL in an `event_record` structured field (same shape used by the pipeline-side fallback), so the payload remains recoverable from journald via `journalctl | jq` and can be re-injected via `limpidctl inject output`. This tracing fallback is strictly worse than a file-based DLQ (log rotation, filters, aggregation delays, no `limpidctl inject` replay shortcut), so operator recovery is fragile without `error_log`. **`SIGKILL` (`kill -9`) does not run this path** — actor tasks are aborted and the stack-local buffer is lost; do not send `SIGKILL` directly to the daemon. See [Queue and retry → Recovery (error_log)](./README.md#recovery-error_log).
- Common queue / retry properties — see [Queue and retry](./README.md#queue-and-retry).
- Client TLS uses tonic's TLS integration (`ClientTlsConfig` backed by rustls with the aws-lc-rs provider). System root certificates are loaded via tonic's `tls-roots` feature; supply per-peer `tls { ca }` to add a custom CA on top.
