...
pub fn run_pipeline(
    ...
    // Per-event arena. A later refactor will start allocating the `Value` tree
    // (HashLits, parser outputs, workspace mutations) into this `Bump`
    // and reset it implicitly when `bump` drops at function return.
    // Today the arena is threaded through call sites but not
...
/// The field is plumbed but unused — `Value` is still
/// owned. A later refactor flips `Value` to `Value<'bump>` and starts using
/// the arena.
