...
function content(page) {
  if (page.kind === "home")
    ...
    <div class="hero-bottom"><p>It is a log pipeline daemon where most of the work is <em>picking which pieces to use</em>.<br>Linux. Windows. macOS.</p><div class="actions"><a class="button" href="${url("docs/getting-started.html")}">Start with the docs <span>→</span></a></div></div>
    </section>
  ...
  if (page.kind === "recipes")
    return `<main id="main" class="landing"><div class="eyebrow">RECIPES / STABLE ${release}</div><h1>Start with<br>a concrete problem.</h1><p class="lede">Configuration recipes for concrete logging problems. Read the assumptions, then adapt the inputs and destinations to your environment.</p><div class="recipe-list"><a href="${url("recipes/fortigate-cef-to-ocsf/")}"><span class="number">00</span><div><span class="eyebrow">DOCUMENTED CONFIGURATION</span><h2>Run the homepage example: FortiGate CEF to OCSF</h2><p>Parse a FortiGate CEF event, compose an OCSF record, and inspect the result locally.</p></div><span>→</span></a></div>${[
      ["Processing", page.recipes.slice(0, 6)],
      ["Destinations", page.recipes.slice(6)],
    ]
      .map(
        ([title, recipes]) =>
...
export default class {
  ...
  render({ entry }) {
    const title = `${entry.title} — limpid`;
    const description = `Readable log pipelines. Documentation and practical configurations for limpid ${release}.`;
    const image = canonical("og.png");
    const imageAlt = "limpid — Log pipelines, limpid as intent.";
    const social = `<meta property="og:title" content="${escape(title)}"><meta property="og:type" content="website"><meta property="og:url" content="${escape(canonical(entry.route))}"><meta property="og:description" content="${escape(description)}"><meta property="og:site_name" content="limpid"><meta property="og:image" content="${escape(image)}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:type" content="image/png"><meta property="og:image:alt" content="${escape(imageAlt)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escape(title)}"><meta name="twitter:description" content="${escape(description)}"><meta name="twitter:image" content="${escape(image)}"><meta name="twitter:image:alt" content="${escape(imageAlt)}">`;
    return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="google-site-verification" content="S-EqEKp48UJAW41lZX5p1lCX1WOcv23Zq_XZxVzEsNk" /><title>${escape(entry.title)} — limpid</title><link rel="canonical" href="${escape(canonical(entry.route))}"><meta name="description" content="Readable log pipelines. Documentation and practical configurations for limpid ${release}.">${social}<link rel="stylesheet" href="${url("style.css")}"><link rel="icon" type="image/svg+xml" href="${url("mark.svg")}"><script type="module" src="${url("copy-code.js")}"></script></head><body><a class="skip" href="#main">Skip to content</a><header><a class="brand" href="${url()}"><img class="brand-mark" src="${url("mark.svg")}" width="42.5" height="37" alt="">limpid<span class="version">v${release}</span></a><nav aria-label="Main"><a href="${url("docs/")}">Docs</a><a href="${url("recipes/")}">Recipes</a><a href="${repository}/releases/tag/v${release}">Downloads ↗</a><a class="github" href="${repository}">GitHub ↗</a></nav></header><div class="page-scroll" tabindex="0" role="region" aria-label="Page content">${content(entry)}<footer><a class="brand" href="${url()}">limpid</a><p>Log pipelines, limpid as intent.</p><a href="${repository}/releases/tag/v${release}">Stable ${release} ↗</a><span>MIT / Apache-2.0</span></footer></div></body></html>`;
  }
}
