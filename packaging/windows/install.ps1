...
function Assert-TrustedWrites([string]$Path, [string[]]$AdditionalTrusted = @(), [switch]$SharedParent) {
    ...
    $writeMask = $writeMask -bor 0x50000000 # GENERIC_WRITE and GENERIC_ALL
    if ($SharedParent) {
        ...
        $writeMask = [int64][Security.AccessControl.FileSystemRights]'Delete, DeleteSubdirectoriesAndFiles, ChangePermissions, TakeOwnership'
        $writeMask = $writeMask -bor 0x10000000
    }
...
if ($InstallDirectory -ne [IO.Path]::GetFullPath((Join-Path $env:ProgramFiles 'limpid'))) { throw 'Custom InstallDirectory is not supported by this package.' }
# The service's diagnostic log and default paths use the platform ProgramData.
if ($DataDirectory -ne [IO.Path]::GetFullPath((Join-Path $env:ProgramData 'limpid'))) { throw 'Custom DataDirectory is not supported by this package.' }
...
if ($existing -and $existing.Status -ne 'Stopped') { throw 'Stop limpid explicitly before upgrading; the installer does not stop an existing service.' }
...
if ($existing) {
    ...
    if (-not $registration.PathName.StartsWith(('"' + $executable + '"'), [StringComparison]::OrdinalIgnoreCase) -or $registration.StartName -ne 'NT SERVICE\limpid') { throw 'Existing limpid service does not match this installation and identity.' }
}
...
if (-not (Get-LocalGroupMember -Group $readers | Where-Object { $_.SID -eq $script:serviceSid })) { Add-LocalGroupMember -Group $readers -Member $script:serviceSid.Value }
